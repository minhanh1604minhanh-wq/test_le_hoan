...
async function speakInstant(text){
  ...
  // Nếu Windows đã có đúng voice VI/EN thì đọc ngay bằng browser: gần như không có delay.
  const voice=getInstantVoice();
  if(voice&&state.audio&&state.lang===requestLang&&speakWithBrowserVoice(cleanText,voice))return;

  // Không có voice đúng ngôn ngữ: dùng OpenAI TTS theo kiểu FAST START.
  // Phần mở đầu ngắn và phần còn lại được tạo SONG SONG.
  const controller=new AbortController();
  ...
  try{
    ...
    const restPromise=rest?fetchTTSBlob(rest,requestLang,controller):null;

    // Chỉ chờ phần đầu ngắn => bắt đầu nghe sớm hơn đáng kể.
    ...
    // Phần sau thường đã được tạo xong trong lúc phần đầu đang đọc.
    if(restPromise&&!controller.signal.aborted&&state.lang===requestLang&&state.audio){
      const restBlob=await restPromise;
      if(!controller.signal.aborted&&state.lang===requestLang&&state.audio){
        await playAudioBlob(restBlob,controller);
      }
    }
  }catch(e){
    if(e?.name!=='AbortError'&&!controller.signal.aborted)console.warn('Fast TTS error',e);
  }finally{
