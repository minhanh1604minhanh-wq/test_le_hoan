...
function getInstantVoice(){
  ...
  const prefix=state.lang==='vi'?'vi':'en';
  return voices.find(v=>(v.lang||'').toLowerCase().startsWith(prefix))||
         voices.find(v=>(v.lang||'').toLowerCase().startsWith(state.lang==='vi'?'en':'vi'))||
         voices[0]||null;
}
function speakInstant(text){
  ...
  if('speechSynthesis' in window&&'SpeechSynthesisUtterance' in window){
    try{
      ...
      const v=getInstantVoice();if(v)u.voice=v;
      u.volume=1;u.rate=state.lang==='vi'?0.97:1;u.pitch=1;
  ...
  // Fallback: chỉ dùng OpenAI TTS nếu trình duyệt không có speechSynthesis.
  speakOpenAIFallback(text);
}
...
function changeLang(){stopCurrentAIWork();pauseNarration(false);state.lang=state.lang==='vi'?'en':'vi';qsa('.result-card').forEach(x=>{x.dataset.filled='';x.innerHTML=''});renderAll();if(state.mainStarted&&state.audio&&!qsa('.panel').some(p=>!p.classList.contains('hidden')))narration().play().catch(()=>{})}
